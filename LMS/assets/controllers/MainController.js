...
app.controller('mainController', ['$scope', 'GraphHelper', '$http', '$location', 'UserService', 'ApiService', function ($scope, GraphHelper, $http, $location, UserService, ApiService) {
    
    $scope.mainController = {};
    $scope.mainController.userDetails = {};
        ...
        if (expiration > new Date()) {
            ...
            if (localStorage.getItem('user') === null) {
                ...
                GraphHelper.me().then(function (response) {
                    ...
                    localStorage.setItem('user', angular.toJson(UserService.getUserDetails()));
                    $scope.getDetails();
                });
            } else {
                UserService.userDetails = angular.fromJson(localStorage.user);
            }
    ...
    $scope.getDetails = function () {
        $scope.mainController.userDetails = UserService.getUserDetails();
        ApiService.getDetails('api/leave', $scope.mainController.userDetails.mail).then(function (data) {
            $scope.mainController.userDetails.firstName = data.data.FirstName;
            $scope.mainController.userDetails.leaveBalance = data.data.LeaveBalance;
        },
        function (error) {
